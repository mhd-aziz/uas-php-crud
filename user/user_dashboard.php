...
$bookmark_stmt = $pdo->prepare("SELECT book_id FROM bookmarks WHERE user_id = ?");
...
<div class="container mt-4 dashboard-container" id="dashboard">
    <div class="dashboard-header d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center mb-4 gap-3">
        ...
        <div class="d-flex flex-wrap gap-2">
            <a href="generate_pdf.php<?= $search ? '?search=' . urlencode($search) : ''; ?>" class="btn btn-secondary btn-responsive" data-bs-toggle="tooltip" data-bs-placement="top" title="Download Daftar Buku">
                <i class="bi bi-file-earmark-pdf"></i> Download PDF
            </a>
        </div>
    </div>
    <?php if (isset($_SESSION['bookmark_message'])): ?>
        <div class="alert alert-success alert-dismissible fade show" role="alert">
            <?= htmlspecialchars($_SESSION['bookmark_message']) ?>
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
        <?php unset($_SESSION['bookmark_message']); ?>
    <?php endif; ?>

    <?php if (isset($_SESSION['bookmark_error'])): ?>
        <div class="alert alert-danger alert-dismissible fade show" role="alert">
            <?= htmlspecialchars($_SESSION['bookmark_error']) ?>
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
        <?php unset($_SESSION['bookmark_error']); ?>
    <?php endif; ?>
                ...
                <?php if ($books): ?>
                    <?php foreach ($books as $index => $book): ?>
                        <tr>
                            ...
                            <td class="text-center align-middle">
                                <!-- Tombol Bookmark -->
                                <?php if (in_array($book['id'], $bookmarked_books)): ?>
                                    <form action="toggle_bookmark.php" method="POST" class="d-inline">
                                        <input type="hidden" name="book_id" value="<?= htmlspecialchars($book['id']) ?>">
                                        <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token']); ?>">
                                        <button type="submit" class="btn btn-warning btn-sm" data-bs-toggle="tooltip" data-bs-placement="top" title="Hapus Bookmark">
                                            <i class="bi bi-star-fill"></i> Unbookmark
                                        </button>
                                    </form>
                                <?php else: ?>
                                    <form action="toggle_bookmark.php" method="POST" class="d-inline">
                                        <input type="hidden" name="book_id" value="<?= htmlspecialchars($book['id']) ?>">
                                        <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token']); ?>">
                                        <button type="submit" class="btn btn-outline-warning btn-sm" data-bs-toggle="tooltip" data-bs-placement="top" title="Tambahkan ke Bookmark">
